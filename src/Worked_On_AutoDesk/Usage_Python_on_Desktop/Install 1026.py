...
def FindFilePath(fileLocation, fileName):
    
    targetFilePath = ''
    
    for file in os.listdir(fileLocation):        
        filePath = os.path.join(fileLocation, file)
        if file == fileName:
            targetFilePath = filePath
            return targetFilePath
        elif os.path.isdir(filePath):
            targetFilePath = FindFilePath(filePath, fileName)
...
if __name__=='__main__':
    ...
    if os.name == 'nt':
        ...
        copyDir(r'{0}\builds\{1}\{2}\NTest3P'.format(remoteDir, label, OS),installPath)
        ...
        copyDir('{0}/builds/{1}/{2}/NTest/NTest'.format(sharePoint, label, OS), os.path.join(installPath, 'Applications'))
        
        #Copy Tests case
        LOG.LogInfo('Start to copy test files on different test directories to local')
        
        if product.lower() != 'cu': #For Main,RC builds to copy test case
            testDirList = ['Fusion', 'Animation', 'FusionDoc', 'NeuCAM', 'Simulation', 'Neutron']
        ...
        elif product.lower() == 'cu': #For CU builds to copy test case
        ...
        LOG.LogInfo('Start to copy NTest 3p files to local')
        # Copy NTest 3p files, such as NastranSolver for run SIM cases
        NTest3POnServer = '{0}/builds/{1}/{2}/NTest3P'.format(sharePoint, label, OS)
        NastranOnServer = os.path.join(NTest3POnServer, 'NASTRANSolver')
        sharedPath = os.path.join(webdeployPath,'shared')
        copyDir(NastranOnServer, sharedPath)

        #Make SymbolicPath
        NastranOnShared = os.path.join(sharedPath, 'NASTRANSolver')
        if os.path.exists(NastranOnShared):
            latestNastranSolverPath = FindFilePath(NastranOnShared, 'NASTRANSolver')
            frameworksSymbolicPath = '{0}/Applications/{1}/Contents/Frameworks'.format(os.environ['HOME'], exeFileName)
            if os.path.exists(latestNastranSolverPath):
                LOG.LogInfo('rm \"' + os.path.join(frameworksSymbolicPath, 'NASTRANSolver') + '\"')
                os.system('rm \"' + os.path.join(frameworksSymbolicPath, 'NASTRANSolver') + '\"')
                LOG.LogInfo('ln -s ' + '\"' + latestNastranSolverPath + '\"' + ' ' + '\"'+ frameworksSymbolicPath + '\"')
                os.system('ln -s ' + '\"' + latestNastranSolverPath + '\"' + ' ' + '\"'+ frameworksSymbolicPath + '\"')
